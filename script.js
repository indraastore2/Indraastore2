...
(function() {
    ...
    // Deteksi Perubahan Ukuran Window (Docked DevTools)
    const detectSize = () => {
        const threshold = 160;
        // Deteksi selisih antara window luar dan dalam
    ...
    //Modern Debugger Trap
    const debuggerCheck = () => {
        const start = Date.now();
        // Perintah debugger akan menahan eksekusi jika DevTools terbuka
    ...
    // Deteksi Eruda & Mobile Inspection Tools
    const erudaCheck = () => {
        // Deteksi objek eruda atau elemen DOM yang disisipkan eruda/chobitsu
        const hasErudaObj = !!window.eruda;
        const hasErudaDom = document.getElementById('eruda') || 
                           document.querySelector('div[class*="eruda"]') || 
                           document.querySelector('div[id*="eruda"]');
    ...
    const setupListeners = () => {
        ...
        document.addEventListener('keydown', e => {
            ...
            const isMacCmdShift = e.metaKey && e.altKey; // Untuk pengguna Mac

            if (
                e.key === 'F12' || 
                (isCtrlShift && (e.key === 'I' || e.key === 'i' || e.key === 'J' || e.key === 'j' || e.key === 'C' || e.key === 'c')) ||
